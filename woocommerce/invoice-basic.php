...
final class RY_IFEZPAY_WC_Invoice_Basic
{
    private static ?self $_instance = null;

    public static function instance(): RY_IFEZPAY_WC_Invoice_Basic
    {
    ...
    public function add_invoice_info($fields)
    {
        $fields['invoice'] = [
            ...
            'invoice_carruer_type' => [
                ...
                'options' => [
                    'ezpay_host' => ry_ifezpay_carruer_type_to_name('ezpay_host') . __(' (send paper when win)', 'ry-invoice-for-ezpay'),
    ...
    public function show_invoice_info($order)
    {
        ...
        $invoice_info[] = [
            ...
            'value' => ry_ifezpay_invoice_type_to_name($invoice_type),
        ];

        switch ($invoice_type) {
            case 'personal':
                $key = count($invoice_info) - 1;
                $invoice_info[$key]['value'] .= ' (' . ry_ifezpay_carruer_type_to_name($carruer_type) . ')';
